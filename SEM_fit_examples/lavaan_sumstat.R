...
# This script fits the model using summary statistics (covariance matrix) see lavaan_rawdata.R 
...
# Fit in Lavaan:
fit1 <- sem(model1, sample.cov = S, sample.nobs = n)
...
# Fit in Lavaan:
fit2 <- sem(model2, sample.cov = S, sample.nobs = n)
...
# Fit in Lavaan:
fit3 <- sem(model3, sample.cov = S, sample.nobs = n)
