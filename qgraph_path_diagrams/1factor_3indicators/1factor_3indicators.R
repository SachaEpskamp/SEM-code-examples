...
size <- 1.2*c(20,10,8,10,8,10,8)
shape <- c("circle","rectangle","circle","rectangle","circle","rectangle","circle")
borders = TRUE
Layout <- matrix(c(
  0,1,
  1,0,
...
labels <- list(expression(eta[1]),expression(y[1]),expression(epsilon[1]),expression(y[2]),expression(epsilon[2]),
               expression(y[3]),expression(epsilon[3]))
eLabs <- list("1","",expression(theta[11]),expression(psi[11]),expression(lambda[21]),"",expression(theta[22]),
              expression(lambda[31]),"",expression(theta[33]))
loopRot <- c(1.5*pi,NA,0.5*pi,NA,0.5*pi,NA,0.5*pi)
